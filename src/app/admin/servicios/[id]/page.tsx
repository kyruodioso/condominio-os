...
export default async function ServiceDetailsPage(props: any) {
    ...
    try {
        ...
        if (!params.id.match(/^[0-9a-fA-F]{24}$/)) {
            return (
                <div className="p-8 text-center text-red-500">
                    <h1 className="text-xl font-bold mb-2">Error de Enlace</h1>
                    <p>El ID del servicio no es válido.</p>
                    <Link href="/admin/servicios" className="text-white hover:underline mt-4 block">Volver</Link>
                </div>
            );
        }
        ...
        return (
            <div className="p-12 text-center text-white bg-red-500/10 rounded-3xl border border-red-500/20">
                <div className="w-16 h-16 bg-red-500/20 text-red-500 rounded-full flex items-center justify-center mx-auto mb-4">
                    <XCircle size={32} />
                </div>
                <h2 className="text-xl font-bold mb-2">Error al cargar el servicio</h2>
                <p className="text-gray-400 mb-6 font-mono text-xs">{error.message}</p>
                <Link href="/admin/servicios" className="inline-block bg-white/10 px-6 py-3 rounded-xl font-bold hover:bg-white/20 transition-colors">
                    Volver al listado
                </Link>
            </div>
        );
